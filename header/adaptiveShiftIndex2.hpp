...
private:
    ...
    inline void distributeLeftSkew(_List_iterator<Block>& blockIt, int32_t delta, int32_t partitionFanoutMask, int32_t partitionFanoutShift, array<Bucket, BUCKETS>& buckets, Timer& timer, ___itt_domain* domain){
        ...
        #pragma unroll
            for (auto i = 0; i < cnt; i++)
            {
                int idx = (((LOG2(data[i])) ^ 3) & partitionFanoutMask) >> partitionFanoutShift;
                bucketsCacheLines[idx].addElement(data[i], buckets[idx]);
            }
    ...
    inline void distributeRightSkew(_List_iterator<Block>& blockIt, int32_t delta, int32_t partitionFanoutMask, int32_t partitionFanoutShift, array<Bucket, BUCKETS>& buckets, Timer& timer, ___itt_domain* domain){
        ...
        #pragma unroll
            for (auto i = 0; i < cnt; i++)
            {
                int idx = (((data[i] ^ 2) * LOG2(data[i])) & partitionFanoutMask) >> partitionFanoutShift;
                bucketsCacheLines[idx].addElement(data[i], buckets[idx]);
            }
    ...
    inline void distributeNormal(_List_iterator<Block>& blockIt, int32_t delta, int32_t partitionFanoutMask, int32_t partitionFanoutShift, int32_t meanVal, int32_t shiftVal, array<Bucket, BUCKETS>& buckets, Timer& timer, ___itt_domain* domain){
        ...
        #pragma unroll
            for (auto i = 0; i < cnt; i++)
            {
                int idx = ((std::signbit(meanVal-data[i]) * ((LOG2(abs(meanVal-data[i]))) ^ 3) + shiftVal) & partitionFanoutMask) >> partitionFanoutShift;
                bucketsCacheLines[idx].addElement(data[i], buckets[idx]);
            }
    ...
    inline double logPartitionSample(vector<int>& data, int pivot){
        auto bucketsFreq = vector<int>(BUCKETS, 0);
    ...
    inline double leftSkewPartitionSample(vector<int>& data, int pivot){
        auto bucketsFreq = vector<int>(BUCKETS, 0);
        auto minData = *std::min_element(data.begin(), data.end());
        auto maxData = *std::max_element(data.begin(), data.end());
        auto minVal = LOG2(minData) ^ 3;
        auto maxVal = LOG2(maxData) ^ 3;
        int dissimilarBit = (DATA_SIZE_BIT - __builtin_clz(minVal ^ maxVal));
        int partitionFanoutShift = dissimilarBit - FANOUT_RADIX;

        if(dissimilarBit > LOG2(CacheBlockSize)){
            for(auto n : data){
                bucketsFreq[((LOG2(n)) ^ 3) >> partitionFanoutShift]++;
            }
        }
        return getVariance(bucketsFreq);
    }

    inline double rightSkewPartitionSample(vector<int>& data, int pivot){
        auto bucketsFreq = vector<int>(BUCKETS, 0);
        auto minData = *std::min_element(data.begin(), data.end());
        auto maxData = *std::max_element(data.begin(), data.end());
        auto minVal = (minData ^ 2) * LOG2(minData);
        auto maxVal = (maxData ^ 2) * LOG2(maxData);
        int dissimilarBit = (DATA_SIZE_BIT - __builtin_clz(minVal ^ maxVal));
        int partitionFanoutShift = dissimilarBit - FANOUT_RADIX;

        if(dissimilarBit > LOG2(CacheBlockSize)){
            for(auto n : data){
                bucketsFreq[((n ^ 2 ) * LOG2(n)) >> partitionFanoutShift]++;
            }
        }
        return getVariance(bucketsFreq);
    }

    inline double normalPartitionSample(vector<int>& data, int pivot){
        auto bucketsFreq = vector<int>(BUCKETS, 0);
        auto meanVal = getMean(data);
        auto minData = *std::min_element(data.begin(), data.end());
        auto maxData = *std::max_element(data.begin(), data.end());
        auto minVal = LOG2(meanVal - minData) ^ 3 + meanVal - minData;
        auto maxVal = LOG2(maxData - meanVal) ^ 3 + meanVal - minData;
        int dissimilarBit = (DATA_SIZE_BIT - __builtin_clz(minVal ^ maxVal));
        int partitionFanoutShift = dissimilarBit - FANOUT_RADIX;

        if(dissimilarBit > LOG2(CacheBlockSize)){
            for(auto n : data){
                bucketsFreq[(std::signbit(meanVal-n) * ((LOG2(abs(meanVal-n))) ^ 3) + meanVal - minVal) >> partitionFanoutShift]++;
            }
        }
        return getVariance(bucketsFreq);
    }
    ...
    inline void partitionLog(_Rb_tree_iterator<pair<const int32_t, Bucket > >& bucketIt, int32_t pivot, Timer& timer, ___itt_domain* domain)
    {
        ...
        if(dissimilarBit > LOG2(CacheBlockSize)){
            ...
            timer.end("partition");
            for(auto it = (bucketIt->second).head.begin(); it != (bucketIt->second).head.end(); ++it){
                // distributeLog(it, 0, 0, 0, buckets, timer, domain);
    ...
    inline void partitionLeftSkew(_Rb_tree_iterator<pair<const int32_t, Bucket > >& bucketIt, int32_t pivot, Timer& timer, ___itt_domain* domain)
    {
        ...
        int32_t partitionMaxVal;
        if ((bucketIt == keyBucketMap.end()) || (next(bucketIt, 1) == keyBucketMap.end())){
            partitionMaxVal = max(LOG2(maxValue) ^ 3, 0);
            if (bucketIt == keyBucketMap.end()){
                bucketIt--;
            }
        }
        else{
            partitionMaxVal = max(LOG2(next(bucketIt, 1)->first) ^ 3, 0);
        }
        int32_t partitionMinVal = max(LOG2(bucketIt->first) ^ 3, 0);
        timer.end("partition");
        ...
        if(partitionFanoutShift > BLOCK_BITS && bucketSize > MIN_BUCKET_SIZE){
            ...
            timer.end("partition");
            for(auto it = (bucketIt->second).head.begin(); it != (bucketIt->second).head.end(); ++it){
                distributeLeftSkew(it, delta, partitionFanoutMask, partitionFanoutShift, buckets, timer, domain);
            }
    ...
    inline void partitionRightSkew(_Rb_tree_iterator<pair<const int32_t, Bucket > >& bucketIt, int32_t pivot, Timer& timer, ___itt_domain* domain)
    {
        ...
        int32_t partitionMaxVal;
        if ((bucketIt == keyBucketMap.end()) || (next(bucketIt, 1) == keyBucketMap.end())){
            partitionMaxVal = max((maxValue ^ 2) * LOG2(maxValue), 0);
            if (bucketIt == keyBucketMap.end()){
                bucketIt--;
            }
        }
        else{
            partitionMaxVal = max((next(bucketIt, 1)->first) ^ 2 * LOG2(next(bucketIt, 1)->first), 0);
        }
        int32_t partitionMinVal = max((bucketIt->first) ^ 2 * LOG2(bucketIt->first), 0);
        timer.end("partition");
        ...
        if(partitionFanoutShift > BLOCK_BITS && bucketSize > MIN_BUCKET_SIZE){
            ...
            timer.end("partition");
            for(auto it = (bucketIt->second).head.begin(); it != (bucketIt->second).head.end(); ++it){
                distributeRightSkew(it, delta, partitionFanoutMask, partitionFanoutShift, buckets, timer, domain);
            }
    ...
    inline FuncPtr selectPartitionFunction(_Rb_tree_iterator<pair<const int32_t, Bucket > > bucketIt, int pivot){
        ...
        auto logPartitionVar = logPartitionSample(samples, pivot);
